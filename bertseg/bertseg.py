...
class BertSeg:
    ...
    def __init__(self, configs, entries):
        ...
        embs = loop.run_until_complete(
            self.get_embeddings([entry["composite"] for entry in self.entries])
        )
        for entry, emb in zip(self.entries, embs):
            entry["embedding"] = emb
            
        # Add an index fieldto the utterance entries
    ...
    async def get_embeddings(self, chunks, model="text-embedding-3-large"):
    ...
    def depth_score_to_topic_change_indexes(
        ...
        return local_maxima_indices[: K - 1]
    ...
    def segment_meeting(
        ...
        depth_score_timeseries = self.depth_score(block_comparison_score_timeseries)
        ...
        transition_points = [pt + self.configs["SENTENCE_COMPARISON_WINDOW"] for pt in transition_points]
